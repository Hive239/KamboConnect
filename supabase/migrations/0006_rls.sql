...
-- session will block all reads/writes. Requires profiles.id == auth.uid() and
...
create policy "practitioners_write" on public.practitioners for all using (id = auth.uid() or public.is_admin()) with check (id = auth.uid() or public.is_admin());

-- events (owner = practitioner_id)
drop policy if exists "events_read" on public.events;
drop policy if exists "events_write" on public.events;
create policy "events_read"  on public.events for select using (true);
create policy "events_write" on public.events for all using (practitioner_id = auth.uid() or public.is_admin()) with check (practitioner_id = auth.uid() or public.is_admin());
...
create policy "reviews_update" on public.reviews for update using (user_id = auth.uid() or practitioner_id = auth.uid() or public.is_admin());
...
create policy "posts_modify" on public.posts for update using (author_id = auth.uid() or public.is_admin());
...
create policy "replies_insert" on public.replies for insert with check (auth.uid() is not null);
create policy "replies_modify" on public.replies for update using (author_id = auth.uid() or public.is_admin());
create policy "replies_delete" on public.replies for delete using (author_id = auth.uid() or public.is_admin());
...
create policy "groups_write" on public.groups for all using (created_by = auth.uid() or public.is_admin()) with check (auth.uid() is not null);
...
create policy "group_memberships_write" on public.group_memberships for all using (user_id = auth.uid() or public.is_admin()) with check (user_id = auth.uid());
...
create policy "feed_items_insert" on public.feed_items for insert with check (auth.uid() is not null);
...
create policy "products_write" on public.products for all using (seller_id = auth.uid() or public.is_admin()) with check (seller_id = auth.uid() or public.is_admin());
...
do $$ declare t text; begin
  foreach t in array array['practitioner_availability','practitioner_blocked_dates','practitioner_exceptions'] loop
    ...
    execute format('create policy %I on public.%I for all using (practitioner_id = auth.uid() or public.is_admin()) with check (practitioner_id = auth.uid() or public.is_admin())', t||'_write', t);
  end loop;
end $$;
...
do $$ declare r record; begin
  for r in select * from (values
    ...
    execute format('create policy %I on public.%I for all using (%I = auth.uid() or public.is_admin()) with check (%I = auth.uid())', r.tbl||'_owner', r.tbl, r.col, r.col);
  end loop;
end $$;
...
do $$ declare r record; begin
  for r in select * from (values
    ...
    execute format('drop policy if exists %I on public.%I', r.tbl||'_rw', r.tbl);
    execute format('create policy %I on public.%I for all using (%I = auth.uid() or %I = auth.uid() or public.is_admin()) with check (%I = auth.uid() or %I = auth.uid() or public.is_admin())',
...
create policy "conversations_twoparty" on public.conversations for all
  using (participant_1_id = auth.uid() or participant_2_id = auth.uid() or public.is_admin())
...
create policy "credentials_owner" on public.credentials for all using (practitioner_id = auth.uid() or public.is_admin()) with check (practitioner_id = auth.uid() or public.is_admin());
...
-- Files are uploaded as "<auth.uid()>/..."; only the owner (or admin) can read/write.
...
create policy "kc_uploads_auth_write"  on storage.objects for insert with check (bucket_id = 'uploads' and auth.uid() is not null);
