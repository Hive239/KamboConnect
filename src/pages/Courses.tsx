...
import { BookMarked, MapPin, Clock, ExternalLink, Plus, Trophy } from "@/lib/icons";
...
const EMPTY = { title: "", provider: "", description: "", url: "", location: "", format: "in_person", duration: "", price: "", lineage: "", image_url: "" };
...
export default function Courses() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const list = (await Course.list("-is_featured")) as any[];
  ...
  const shown = filter === "all" ? courses : courses.filter((c) => c.format === filter);

  const save = async () => {
    if (!form.title.trim()) { toast.error("Title is required."); return; }
    setSaving(true);
    try {
      await Course.create({ ...form, price: form.price ? Number(form.price) : null, status: "published" });
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {isAdmin && (
              <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild><Button className="gap-2"><Plus className="h-4 w-4" /> Add course</Button></DialogTrigger>
                <DialogContent className="max-h-[85vh] overflow-y-auto">
                  ...
                  <div className="space-y-3">
                    ...
                    <Button onClick={save} disabled={saving} className="w-full">{saving ? "Saving…" : "Add course"}</Button>
                  </div>
                </DialogContent>
              </Dialog>
            )}
        ...
        {loading ? (
          ...
          <Card><CardContent className="py-16 text-center text-muted-foreground">
            No courses listed yet{filter !== "all" ? " for this format" : ""}. Check back soon.
          </CardContent></Card>
        ) : (
              ...
              <Card key={c.id} className="flex flex-col overflow-hidden">
                ...
                <CardContent className="flex flex-1 flex-col gap-3 p-5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {c.is_featured && <Badge className="shrink-0 gap-1 bg-amber-100 text-amber-800"><Trophy className="h-3 w-3" /> Featured</Badge>}
                  </div>
